class Paper {
    constructor(canvas) {
        ...
        this.floatingPointCorrectionFactor = 10;
        this.drawAxes();
    }
    ...
    drawGraph(f, color, dots) {
        console.log("Drawing "+(color||this.defaultGraphColor)+" graph"+(dots ? " with dots" : "")+" in interval " + this.x_min() + " < x < " + this.x_max()+"...");
        var x;
        var dx = this.dx;
        var cf = this.floatingPointCorrectionFactor;
        this.setColor(color);
        this.context.beginPath();
        // *2 so the graph does not appear cut off at the edges:
        for (x = Math.floor(this.x_min())*2; x <= Math.ceil(this.x_max())*2; x = (x*cf + dx*cf)/cf) {
    ...
    drawDerivative(f, color, dots) {
        var self = this;
        var f_prim = function(x) {
            return self.deriv(f, x);
        }
        this.drawGraph(f_prim, color, dots);
    }

    drawSecondDerivative(f, color, dots) {
        var self = this;
        var f_prim = function(x) {
            return self.deriv(f, x);
        }
        this.drawDerivative(f_prim, color, dots);
    }
...
if (canvas instanceof HTMLCanvasElement) {
    var examplePaper = new Paper(canvas);
}

function drawPoint(x, y, color) {
    examplePaper.drawPoint(x, y, color);
}

function drawLine(x1, y1, x2, y2, color) {
    examplePaper.drawLine(x1, y1, x2, y2, color);
}

function drawGraph(f, color) {
    examplePaper.drawGraph(f, color, false);
}

function drawGraphWithPoints(f, color) {
    examplePaper.drawGraph(f, color, true);
}

function drawDerivative(f, color) {
    examplePaper.drawDerivative(f, color, false);
}

function drawDerivativeWithPoints(f, color) {
    examplePaper.drawDerivative(f, color, true);
}

function drawSecondDerivative(f, color) {
    examplePaper.drawSecondDerivative(f, color, false);
}

function drawSecondDerivativeWithPoints(f, color) {
    examplePaper.drawSecondDerivative(f, color, true);
}

function setScale(s) {
    examplePaper.setScale(s);
}

function setPrecision(p) {
    examplePaper.setPrecision(p);
}

function saveAsImage() {
    window.open(examplePaper.toImage());
}

function saveAsImageWithBackground(color) {
    window.open(examplePaper.toImage((color || "white")));
}
